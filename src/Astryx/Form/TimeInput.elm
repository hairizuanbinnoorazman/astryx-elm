module Astryx.Form.TimeInput exposing (Config, view)

{-| Native time input with Field wiring.
...
type alias Config msg =
    { wiring : Wiring, value : String, step : Maybe Int, disabled : Bool, readOnly : Bool, onInput : String -> msg, attributes : List (Attribute msg) }


{-| Render a browser-native time control.
-}
        ...
        (Attributes.merge
            ([ Attr.class "astryx-input"
             ...
             , Events.onInput config.onInput
                ...
                ++ (config.step |> Maybe.map (\value -> [ Attr.step (String.fromInt value) ]) |> Maybe.withDefault [])
            )
            config.attributes
        )
        []
